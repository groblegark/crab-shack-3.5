...
| STREAM CURSOR | **LANDED** (owner ruling 2026-08-22 "we gotta definitely keep the same seed"; byte-identical by gate — frozen fingerprints unmoved both backends, 16-seed matrix byte-identical to e5d03c1, 283/283 exit 0 armed and unarmed, kernel/shared-cursor/draw-count pins green) | the SIM CURSOR IS TOWN STATE: the envelope carries `rs` (the mulberry32 cursor, one u32, written only when the town owns its stream) and `sd` (the founding seed, provenance). `SAVE_VER` UNCHANGED at 3 — both fields additive and optional. Unarmed the cursor is module-scoped `_rs` (out of its closure so the envelope can read it); armed it is `KRNG[0]`, a view over kernel memory at 26624 — one logical cell, two homes, one door (`simCursor`/`simStreamAdopt`). The VIEW stream (`vrand`) stays SESSION state: unsaved, unrestored, pinned by scenario | `load()` adopts the cursor **BEFORE `resetSession()`**, because resetSession rebuilds the townsfolk through `initNpcs()` and initNpcs DRAWS — adopted after, the crabs were still minted from the session's leftovers, the whole bug surviving inside its own fix (caught by the scenario, not by reading). A pre-cursor save DERIVES one by FNV-1a over its own stable bytes: not the future it would have had, but the same future every time. `makeCrabPersona`'s rng now defaults to `srand` not `Math.random` — fingerprint-neutral (both consumers already shared one cursor) but load-critical (a post-load hire drew from the session otherwise), pinned by an ESCALATED data pin after the trajectory proved it vacuous. **Boot still uses the host stream** — a fresh town has no save to replay; seeding at boot is a bigger landing and its measurements are in the close-out |
| NEURO LADDER 1-4 | **LANDED** (owner: "proceed" + crab brain LIVE by ruling; the full re-baseline paid — first crossing NAMED and stable across artifact generations, nine pins walked by mechanism, baseline 0/48 medians in band, growth reported honestly at 8/48 vs 13/48 with the act-early attribution; suite 289/289 exit 0 both backends) | BRAINS are integer arithmetic under the standing contract: int8 weights, int16 activations, exact int32 accumulation, ONE floor per layer at a named shift, saturating ReLU, argmax with lowest-index tie-break — every intermediate < 2^28, bit-identical across engines by the same argument as money. A brain's FEATURE VECTOR IS DATA: ordered picks from the versioned observable registry (v1: 12 named + 6 parameterized × 5 stops; unknown names fail loudly), assembled at think time by reads in declared order. **Brains are DRAW-FREE**: they rank what `visCandidates` built, so the stream leaves in the same order whoever decides — the pairing pin proves it and its mutation bites at think 3. Policies per surface: `table | script | brain` × `off | shadow | live`; hostile-file caps enforced at the door with the offending number in the message (inputs ≤64, hidden ≤256, params ≤32K, MACs ≤64K — the caps are LAYERED and the params wall is unreachable under today's shapes, recorded not claimed) | the crab default and the gulls ship LIVE (95.7%/94.9% held-out; 42→24→7, 1.3 KB each); shadow mode is standing infrastructure, proven inert by pin; the kernel DEFERS for brain-carrying deciders identically in both modes (cost says JS-side: 0.0391 thinks/tick × ~1µs ≈ noise; the kernel port waits behind a measured trigger); mcp policy_distill/verify close the authoring loop (artifacts arrive SHADOW; promotion is the author's act and pays the re-baseline); the draw-count spec re-pointed 1857/2265 behind the traced NIPPY head |
