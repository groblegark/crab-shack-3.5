...
const KERNEL = argv.includes("--kernel") ? argv[argv.indexOf("--kernel") + 1] : undefined;   // wasm|off (the movement-kernel spike)
...
const round = () => {
  ...
  for (let i = 0; i < SEEDS; i++) {
    const seed = seedOf(i);
    const sim = createSim({ seed, realm: REALM, kernel: KERNEL });
...
if (JSON_OUT) {
  console.log(JSON.stringify({
    simDays, realm: REALM || process.env.SIMLIB_REALM || "vm", kernel: KERNEL || process.env.SIMLIB_KERNEL || "off", rounds: REPEAT, cpuSec: +cpuSec.toFixed(2), wallSec: +wallSec.toFixed(2),
