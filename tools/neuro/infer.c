/* THE INTEGER INFERENCE RECIPE, compiled — infer.mjs's twin, for the wasm
   ...
   Memory map (imported nothing; own 4MB memory, runner writes then calls):
     16384    i32[4]   dims: NF, HID, NC, R1
     17408    i8[]     w1   (HID x NF, row-major)
     81920    i32[]    b1   (HID)
     131072   i8[]     w2   (NC x HID, row-major)
     196608   i32[]    b2   (NC)
     262144   i16[]    corpus (n rows x NF)
     3145728  i32[]    out logits (n rows x NC)
...
static int32_t *const OUT  = (int32_t *)3145728;
