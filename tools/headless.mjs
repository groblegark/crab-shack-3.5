...
// --noswim is the swim economy's zero-dose twin: it arms off the DS.atSwim
// relief errand (window._noSwim) so the matrix can price the swim cure by
// difference. Visual only lives elsewhere (--noseacues); this moves behaviour.
const NOSWIM = args.includes("--noswim");
...
// `--citnoworkpause` turns OFF the on-duty pause (crabTick pauses the citizen
// drain while working/commuting by default - work is "occupied", no relief path,
// the calibrated model). This is the attribution arm for the pause: game.js
// reads window._citNoWorkPause in crabTick and never sets it.
const CITNOWORKPAUSE = args.includes("--citnoworkpause");
...
if (NOSWIM) G(`window._noSwim = true;`);
